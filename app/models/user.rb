class User < ApplicationRecord
  ...
  def favorite(micro_post)
      self.favorites.find_or_create_by(micropost_id: micro_post.id)
  end
  
  def unfavorite(micro_post)
    myfavorite = self.favorites.find_by(micropost_id: micro_post.id)
    myfavorite.destroy if myfavorite
  end
  
  def favorite?(micro_post)
    self.likes.include?(micro_post)
  end  
